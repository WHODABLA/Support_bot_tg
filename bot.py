...
# Initialize Groq client with explicit API key and without proxies
GROQ_API_KEY = os.environ.get("GROQ_API_KEY")
if not GROQ_API_KEY:
    logger.warning("GROQ_API_KEY not set in environment variables")

# Create client with explicit API key and no proxies
client = Groq(
    api_key=GROQ_API_KEY,
    # Disable proxies to avoid the compatibility issue
    http_client=None  # This forces the client to use default settings without proxies
)
